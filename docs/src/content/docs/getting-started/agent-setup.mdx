...
<Aside type="note">
The `-v ".:/data"` flag mounts your current project directory as `/data` inside the container. Sheriff's working directory is `/data`, so:
- **SARIF paths** are relative to your project root (e.g., `load target="build/results.sarif"`)
- **State directory** (`.sheriff/`) is created inside your project directory via the mount
- **Export files** are written to your project directory
</Aside>
  </TabItem>
</Tabs>
