...
<Aside type="note">
**Severity naming convention:** Use full names (`High`, `Moderate`, `Low`) in scope filter **input**. Sheriff uses single-letter abbreviations (`H`, `M`, `L`) in response **output** to minimize token usage.
</Aside>
...
<Aside type="tip">
The abbreviated format significantly reduces context usage compared to verbose field names.
</Aside>
