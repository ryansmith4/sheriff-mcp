...
<Aside type="note">
Unlike `next`, `progress`, and `export`, `summary` does not support scope filtering. It always shows the complete breakdown across all issues, which makes it ideal for initial planning and prioritization. Use `progress` with a scope filter to check status within a specific category.
</Aside>
